...
public class TSQL2ResultSet implements ResultSet, Constants {
	/**
	 * Flag for activating debug mode. In debug mode, temporal columns can be accessed directly.
	 */
	public static boolean DebugMode = false;
	/**
	 * Original relational result set with results from relational database.
	 */
	private ResultSet _originalResults;
	/**
	 * Result set metadata for original result set.
	 */
	private TSQL2ResultSetMetaData _metadata;
	...
	private void checkColumnLabel(String columnLabel) throws TSQL2Exception {
		if (TSQL2ResultSet.DebugMode) return;
	...
	public String getString(int columnIndex) throws SQLException {
		...
		if (_metadata.getColumnTSQLType(columnIndex) == TSQL2Types.PERIOD) {
		...
		} else if (_metadata.getColumnTSQLType(columnIndex) == TSQL2Types.EVENT) {
