...
public class Utils implements Constants {
	/**
	 * Enclose identifier in quotes for current DBMS.
	 * This should be used for column or table names for statements.
	 * 
	...
	/**
	 * Enclose string in quotes for current DBMS.
	 * This should be used for plain strings for statements.
	 * 
	 * @param str String to enclose
	...
	/**
	 * Convert time from unix timestamp to string representation
	 * with specified resulting scale.
	 * 
	 * @param seconds Time in seconds in Unix timestamp
	...
	public static String timeToString(long seconds, DateTimeScale scale)
	{
		...
		String result = "";
		if (scale.equals(DateTimeScale.YEAR)) {
			result = String.format("%04d", 
					 	c.get(Calendar.YEAR));
		} else if (scale.equals(DateTimeScale.MONTH)) {
			result = String.format("%04d-%02d", 
				 	c.get(Calendar.YEAR), 
					c.get(Calendar.MONTH)+1);
		} else if (scale.equals(DateTimeScale.DAY)) {
			result = String.format("%04d-%02d-%02d", 
				 	c.get(Calendar.YEAR), 
					c.get(Calendar.MONTH)+1, 
					c.get(Calendar.DAY_OF_MONTH));
		} else if (scale.equals(DateTimeScale.HOUR)) {
			result = String.format("%04d-%02d-%02d %02d:%02d", 
					...
					0);
		} else if (scale.equals(DateTimeScale.MINUTE)) {
			result = String.format("%04d-%02d-%02d %02d:%02d", 
				 	c.get(Calendar.YEAR), 
					c.get(Calendar.MONTH)+1, 
					c.get(Calendar.DAY_OF_MONTH), 
		...
		} else if (scale.equals(DateTimeScale.SECOND)) {
	 ...
	 * 
	 * @param date String representing date in format YYYY[-MM[-DD[ HH[:MM[:SS]]]]]
