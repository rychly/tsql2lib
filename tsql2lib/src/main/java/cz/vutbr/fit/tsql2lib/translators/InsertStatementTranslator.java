...
public class InsertStatementTranslator extends TranslatorBase {
	 ...
	 * Insert statement sometimes needs to create SelectTranslator for subquery so it needs
	 ...
	 */
	private ArrayList<String> _insertColumns = new ArrayList<String>();
	/**
	 * Insert  values
	 */
	private ArrayList<String> _insertValues = new ArrayList<String>();
	/**
	 * If insert is not with values but with subselect, store subselect tree node here.
	 * Subselect statement will be translated from node when all required data are collected
	 * (temporal support, columns, ...).
	 */
	private SimpleNode _insertSubselect = null;
	/**
	 * Columns for temporal data
	 */
	private ArrayList<String> _tempColumns = new ArrayList<String>();
	/**
	 * Values of temporal data
	 */
	private ArrayList<String> _tempValues = new ArrayList<String>();
	/**
	 ...
	 */
	private long _transStart = Utils.getCurrentTime();
	/**
	 * End of transaction time for inserted data
	 */
	private long _transEnd = FOREVER;
	/**
	...
	public String[] translate(SimpleNode treeRoot) throws TSQL2TranslateException {
		...
		for (int i = 0; i < treeRoot.jjtGetNumChildren(); i++) {
			...
			} else if (nodeType == "InsertColumns") {
				processInsertColumns(node);
			} else if (nodeType == "InsertValues") {
		...
		String[] sArr = new String[1];
                System.err.println(statements.toString());
		return statements.toArray(sArr);
	}

	/**
	 * Insert statement may need temporal values, depending on table temporal support.
	 * If temporal values were not specified explicitly, this method sets them
	 * to default values.
	 */
	...
	private void processValidExpression(SimpleNode treeRoot) throws TSQL2TranslateException {
		...
		if (_tableInfo.getValidTimeSupport().equals(STATE)) {
			if (nodeType == "PeriodExpression") {
				...
				_tempColumns.add(Settings.ValidTimeEndColumnName);
				_tempValues.add(String.valueOf(_validEnd));
			} else if ((nodeType == "DateExpression") 
					|| (nodeType == "TimeExpression")
					|| (nodeType == "TimestampExpression")) {
